{{ config(
    post_hook=[
         "alter table {{ this }} add constraint fk_dim_date_witness_key foreign key (dim_date_witness_key) references {{ ref('dim_date') }} (dim_date_key)"
        ,"alter table {{ this }} add constraint fk_dim_date_agent_key foreign key (dim_date_agent_key) references {{ ref('dim_date') }} (dim_date_key)"
        ,"alter table {{ this }} add constraint fk_dim_witness_key foreign key (dim_witness_key) references {{ ref('dim_witness') }} (dim_witness_key)"
        ,"alter table {{ this }} add constraint fk_dim_agent_key foreign key (dim_agent_key) references {{ ref('dim_agent') }} (dim_agent_key)"
        ,"alter table {{ this }} add constraint fk_dim_location_key foreign key (dim_location_key) references {{ ref('dim_location') }} (dim_location_key)"
    ]
